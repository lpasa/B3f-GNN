...
import datetime
import torch
from Boosting.boostingClassNetwork import classNetwork

class B3FGNN(torch.nn.Module):
    def __init__(self, in_channels,n_class, n_neurons_per_classifier, n_classifiers_per_layer, n_hidden_layers ,context_dim, n_prototypes, conv, conv_act=lambda x:x, H_as_context = False, device=None):
        super(B3FGNN, self).__init__()
        if device is None:
        ...
        for c in range(n_class):
            self.class_networks.append(classNetwork(in_channels=in_channels,
                                                    ...
                                                    conv_act=conv_act,
                                                    device=device,
                                                    H_as_context = H_as_context))

        self.out_fun=torch.nn.Softmax(dim=1)
    ...
    def train_and_test(self, data, train_mask, test_mask, valid_mask,epochs,context=None,test_name="",
                log_path=".", max_n_epochs_without_improvements=30, early_stopping_threshold=0, test_epoch=1,
                       set_layer_proto_dynamically=False, balanced_training=False):
        h_last_layers=[]
